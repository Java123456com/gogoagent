...
> **环境不可用时的唯一处置**：在项目根目录执行 `npm install`，将 `tuniu-cli` 安装到项目自己的 `node_modules`。Python Agent 会自动把项目 `node_modules/.bin` 加入 PATH。**不要**执行全局安装、手工设置 `npm root -g`、使用绝对路径或 `npx tuniu-cli`。
